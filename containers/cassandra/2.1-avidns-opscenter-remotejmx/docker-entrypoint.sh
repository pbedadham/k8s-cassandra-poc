...
while [  "$BROADCAST_IP" = "" ]; do
	BROADCAST_IP=`dig ${CASSANDRA_NODE_NAME}.${K8S_NAMESPACE}.${AVI_DOMAIN_NAME} @${AVI_DNS_VIP} +short`
	sleep 5
done
CASSANDRA_BROADCAST_RPC_ADDRESS=$BROADCAST_IP
CASSANDRA_BROADCAST_ADDRESS=$BROADCAST_IP
#set broadcast rpc address to avi dns
SEED_IP=""
while [  "$SEED_IP" = "" ]; do
	SEED_IP=`dig ${CASSANDRA_SEEDS}.${K8S_NAMESPACE}.${AVI_DOMAIN_NAME} @${AVI_DNS_VIP} +short`
	sleep 5
done
